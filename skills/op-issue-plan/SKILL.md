...
4. **Data classification** (HIPAA / privacy): state whether this change touches PHI, PII, or
   otherwise-regulated data. If yes, note the data flow and where it is stored, logged, or sent to a
   model. Flag any path that would put regulated data into logs, prompts, fixtures, or a third-party service.
